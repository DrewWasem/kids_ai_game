...
### research/
Structured research documents from the `/research` command. Each document is self-contained with findings, code references, and open questions. Used as input for `/create-plan`.

### plans/
Implementation plans from `/create-plan`. Plans are living documents — checkboxes get updated during `/implement-plan`. Status field tracks: draft → approved → in-progress → completed.
