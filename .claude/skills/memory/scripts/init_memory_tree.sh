...
EOF
    ...
    if ! grep -q "research/" "$MEMORY_DIR/_index.md" 2>/dev/null; then
        echo "| research/ | Codebase research documents (from /research) | — |" >> "$MEMORY_DIR/_index.md"
        echo "  Added research/ to root index"
    fi
    if ! grep -q "plans/" "$MEMORY_DIR/_index.md" 2>/dev/null; then
        echo "| plans/ | Implementation plans (from /create-plan) | — |" >> "$MEMORY_DIR/_index.md"
...
| research/ | Codebase research documents (from /research) | — |
...
Codebase research documents from the /research command. Each document is self-contained with findings, code references, and open questions.
...
Implementation plans from /create-plan. Plans are living documents with progress checkboxes. Status: draft → approved → in-progress → completed.
...
echo "Run /memory-status to browse, /remember to capture, /recall to retrieve."
echo "Use /conductor for orchestrated Research -> Plan -> Implement workflows."
